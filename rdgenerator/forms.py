...
class GenerateForm(forms.Form):
    ...
    enableAudio = forms.BooleanField(initial=True, required=False, label="启用音频传输")
    ...
    enablePrinter = forms.BooleanField(initial=True, required=False, label="启用远程打印")
    ...
    hidePassword = forms.BooleanField(initial=False,required=False,label="隐藏密码框(仅允许传入连接模式下选择)")
    hideMenuBar = forms.BooleanField(initial=False,required=False,label="隐藏菜单(仅允许传入连接模式下选择)")
